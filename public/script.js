...
function applyTheme(theme) {
  ...
  if (isLight) {
    ...
    document.querySelectorAll('.nav-logo, .theme-logo').forEach(img => {
      img.src = '/logo_rcp_fondo_claro.svg?v=1.0.2-black';
    });
  } else {
    ...
    document.querySelectorAll('.nav-logo, .theme-logo').forEach(img => {
      img.src = '/logo_rcp_fondo_oscuro.svg?v=1.0.2-black';
    });
  }
}
...
(function () {
  ...
  const GREETING_ES = `¡Hola! Soy Pulso, el asistente virtual de RCP Services. ¿En qué te puedo ayudar hoy?`;
  ...
  const faq = [
    ...
    },
    {
      ...
      a: '¡Hola! 👋 ¡Qué bueno saludarte! Soy <strong>Pulso</strong>, el asistente virtual de RCP Services. ¿De qué trata tu negocio? 😊',
